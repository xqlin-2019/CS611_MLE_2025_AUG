        ...
        if name == 'loan_daily':
            ...
            for i in range(1, 21):
                ...
                df = df.withColumn(f"fe_{i}", 
                    F.when(col(f"fe_{i}") < 0, 0).otherwise(col(f"fe_{i}"))
                )
            ...
            df = df.withColumn("Occupation", F.when(col("Occupation") == "_______", None)
                                                                          .when(col("Occupation") == "Media_Manager", "Media Manager")
            ...
            # Remove garbage characters in behaviour fields
            df = df.withColumn("Payment_Behaviour", 
                F.when(~F.col("Payment_Behaviour").rlike("^[A-Za-z_]+$"), None).otherwise(col("Payment_Behaviour"))
            )

            df = df.withColumn("Num_Bank_Accounts", 
                F.when(col("Num_Bank_Accounts") > 20, 20).otherwise(col("Num_Bank_Accounts"))
            )
